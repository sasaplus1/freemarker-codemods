...
function extractIncludePath(includePath: string): string {
...
export function resolveInclude(
  ...
  if (/^\.{1,2}\//.test(includePath)) {
    ...
    resolvePaths = paths.map((resolvePath) =>
      path.join(resolvePath, path.dirname(includePath))
    );
  }
  ...
  for (const token of tokens) {
    ...
    if (isIncludeToken(token) && typeof params === 'string') {
      const includeTokens = resolveInclude(
        extractIncludePath(params),
        { ...options, recursive: options.recursive - 1 },
        { ...codemods, filePath: templateFile }
      );
